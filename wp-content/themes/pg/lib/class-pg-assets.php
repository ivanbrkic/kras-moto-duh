...
final class Pg_Assets {
	 ...
	 * @var string
	 */
	protected static $theme_img_dir_path = '/theme/img';
	...
	private static function set_assets_paths() : void {
		self::$assets_directory     = get_template_directory() . self::$assets_dist_dir_path;
		self::$assets_directory_uri = get_template_directory_uri() . self::$assets_dist_dir_path;
	}
	...
	public static function get_asset_version( string $asset_key ) : ?string {
		...
		$asset_data = ! empty( $mix_manifest_data[ $asset_key ] ) ? $mix_manifest_data[ $asset_key ] : null;
	...
	public static function get_asset_path( string $asset_path ) : string {
		return self::$assets_directory . $asset_path;
	}
	...
	private static function read_json( string $json_path ): ?array {
		// We're have to use file_get_contents() in order to read the JSON so we're disabling PHP_CodeSniffer rule.
	 ...
	 * @return array
	 */
	private static function get_json( string $json_path ) : array {
	...
	private static function set_mix_manifest_data() : void {
		self::$mix_manifest_data = self::get_json( self::$assets_directory . '/mix-manifest.json' );
	}
	...
	private static function set_icon_list() : void {
		if ( self::asset_exists( self::$sprite_path ) ) {
			...
			if ( $sprite_xml && isset( $sprite_xml->defs ) ) {
				foreach ( $sprite_xml->defs->children() as $val ) :
	...
	public static function get_theme_img( string $img_path, array $img_attr = [] ) : string {
		// Return the img.
		return self::get_img( self::$theme_img_dir_path . '/' . $img_path, $img_attr );
	}
	...
	public static function get_theme_svg( string $svg_path, bool $append_extension = true ) : string {
		return self::get_svg( self::$theme_img_dir_path . '/' . $svg_path, $append_extension );
	}
}
